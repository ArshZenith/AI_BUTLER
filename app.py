...
def render_ultimate_ui(active_mode, matrix_mode=False, theme_mode="dark", current_theme="royal"):
    ...
    sidebar_bg_end = 'rgba(245,245,245,0.99)' if theme_mode == 'light' else 'rgba(10,10,15,0.99)'
    input_bg = 'rgba(255,255,255,0.9)' if theme_mode == 'light' else 'rgba(20,20,30,0.8)'
    btn_color = '#000000'
    sidebar_btn_bg = 'rgba(255,255,255,0.8)' if theme_mode == 'light' else 'rgba(20, 20, 30, 0.7)'
    sidebar_btn_hover = 'rgba(240,240,240,0.95)' if theme_mode == 'light' else 'rgba(40, 40, 50, 0.9)'
    stat_label_color = '#666' if theme_mode == 'light' else '#888'
    toast_color = '#000'
    scrollbar_track = '#f0f0f0' if theme_mode == 'light' else '#0a0a0f'
    select_bg = 'rgba(255,255,255,0.9)' if theme_mode == 'light' else 'rgba(20,20,30,0.8)'
    rename_bg = '#fff' if theme_mode == 'light' else '#1a1a2e'
    
    greeting_bg = 'linear-gradient(135deg, rgba(255,255,255,0.8), rgba(245,245,245,0.9))' if theme_mode == 'light' else 'linear-gradient(135deg, rgba(30,30,40,0.6), rgba(20,20,30,0.8))'
    greeting_text = '#666' if theme_mode == 'light' else '#888'
    greeting_subtext = '#888' if theme_mode == 'light' else '#aaa'
    footer_color = '#666' if theme_mode == 'light' else '#999'
    ...
    <style>
        ...
        * {{ cursor: none; }}
        ...
        .folder-name {{ flex-grow: 1; font-weight: 600; }}
        .folder-count {{ background: {primary_color}30; padding: 2px 8px; border-radius: 10px; font-size: 0.8em; font-weight: 700; }}
    </style>
...
if "active_mode" not in st.session_state: st.session_state.active_mode = "butler"
if "last_processed_audio" not in st.session_state: st.session_state.last_processed_audio = None
if "toast_message" not in st.session_state: st.session_state.toast_message = None
...
if "voice_speed" not in st.session_state: st.session_state.voice_speed = "normal"
if "voice_lang" not in st.session_state: st.session_state.voice_lang = "en"
if "timer_minutes" not in st.session_state: st.session_state.timer_minutes = 25
if "timer_running" not in st.session_state: st.session_state.timer_running = False
if "timer_end_time" not in st.session_state: st.session_state.timer_end_time = None
if "selected_tool" not in st.session_state: st.session_state.selected_tool = "💬 Normal Chat"
if "gmail_authenticated" not in st.session_state: st.session_state.gmail_authenticated = False
if "current_persona" not in st.session_state: st.session_state.current_persona = "default"
if "renaming_chat_id" not in st.session_state: st.session_state.renaming_chat_id = None
if "current_folder" not in st.session_state: st.session_state.current_folder = "all"
...
if not st.session_state.user_authenticated:
    ...
    with col2:
        ...
        email_input = st.text_input("ENTER COMM-LINK (Email):", placeholder="operator@domain.com", key="login_email")
...
# ========================================
with st.sidebar:
    header_border = '#FFD700' if st.session_state.theme_mode == 'dark' else '#B8860B'
    header_color = '#FFD700' if st.session_state.theme_mode == 'dark' else '#B8860B'
    
    st.markdown(f"""
    <div style='text-align: center; padding: 20px 0; border-bottom: 2px solid {header_border}40; margin-bottom: 20px;'>
        ...
        <h2 style='font-family: Playfair Display; color: {header_color}; margin: 0; font-size: 1.8rem;'>{st.session_state.username}</h2>
    ...
    theme_options = {"royal": "👑 Royal Gold", "cyberpunk": "🤖 Cyberpunk", "neon": "🌸 Neon Glow", "ocean": "🌊 Ocean Blue", "forest": "🌲 Forest Green"}
    ...
    try:
        ...
        for name, symbol in tickers.items():
            try:
                ...
                st.markdown(f"""<div class="ticker-card"><span class="ticker-symbol">{name}</span><span class="ticker-price">${current_price:,.2f}</span></div>""", unsafe_allow_html=True)
            except: pass
    except: pass
    ...
    if matrix_toggle != st.session_state.matrix_mode:
        ...
        st.rerun()
    st.components.v1.html(f"""<script>window.parent.postMessage({{type: 'matrix_toggle', active: {str(matrix_toggle).lower()}}}, '*');</script>""", height=0)
    
    st.divider()
    st.markdown("<h4 style='color:#FFD700; margin-bottom: 15px;'>⏱️ TIMER</h4>", unsafe_allow_html=True)
    timer_mins = st.number_input("Minutes:", min_value=1, max_value=120, value=st.session_state.timer_minutes, key="timer_input", disabled=st.session_state.timer_running)
    if timer_mins != st.session_state.timer_minutes and not st.session_state.timer_running: st.session_state.timer_minutes = timer_mins
    ...
    for folder_id, folder_data in folders.items():
        if folder_data.get("is_system", False) or folder_id == "all": continue
        ...
        with col_f1:
            if st.button(f"{folder_data['icon']} {folder_data['name']}", key=f"folder_{folder_id}", use_container_width=True, type="primary" if is_selected else "secondary"):
        ...
        with col_f2:
            if st.button("🗑️", key=f"del_folder_{folder_id}"):
                ...
                if st.session_state.current_folder == folder_id: st.session_state.current_folder = "all"
                st.rerun()
    ...
    for chat_id in chat_ids[:15]:
        ...
        if st.session_state.renaming_chat_id == chat_id:
            new_name = st.text_input("New name:", value=chat_data.get("title", "Chat"), key=f"rename_{chat_id}")
    ...
    with col_s2:
        st.markdown(f"""<div class="stat-card"><div class="stat-value">{user_stats.get('total_messages', 0)}</div><div class="stat-label">Messages</div></div>""", unsafe_allow_html=True)
    ...
    export_format = st.selectbox("Export Format:", ["txt", "json", "markdown", "html"], key="export_format")
    if st.button("📥 Export Chat", use_container_width=True):
        ...
        st.download_button("⬇️ Download", content, file_name=f"chat_export.{file_ext}", use_container_width=True)
...
if st.session_state.selected_tool != "💬 Normal Chat":
    try:
        ...
        if result == "feature_rendered": st.stop()
...
st.markdown(f"""
<div style='text-align: center; margin-bottom: 30px; padding: 30px; background: {greeting_bg}; border-radius: 20px; border: 2px solid {current_mode_config['color']}40; backdrop-filter: blur(20px);'>
    ...
    <p style='color:{greeting_text}; font-size:1.2rem; margin: 10px 0;'>{now.strftime("%A, %d %B %Y")} | {now.strftime("%I:%M %p")} IST</p>
    <p style='color:{greeting_subtext}; font-size:1rem; margin: 10px 0 0 0;'>Mode: {current_mode_config['name']} | Persona: {persona_options[st.session_state.current_persona]}</p>
</div>
""", unsafe_allow_html=True)
...
if final_prompt:
    ...
    with st.chat_message("assistant"):
        ...
        if response_text and response_text.strip():
            ...
            suggestions = ["Explain this in simpler terms", "Give me more examples", "What are the pros and cons?", "How can I apply this practically?"]
            cols = st.columns(4)
