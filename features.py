...
def render_selected_tool(tool_name):
    ...
    # ========================================
        # ========================================
        # ========================================
    # 📸 MAGIC VISION (POLLINATIONS.AI ALTERNATIVE)
    # ========================================
            ...
            if st.button("📸 Analyze", type="primary", use_container_width=True):
                try:
                    ...
                    with st.spinner("📸 Analyzing image..."):
                        ...
                        # Use Pollinations.ai Vision API (Free, No Key Required)
                        api_url = "https://image.pollinations.ai/prompt/describe"
                        
                        # Alternative: Use Llama 3.2 via Pollinations
                        # For better accuracy, we'll use a text-based approach with image description
                        # Since direct vision API might vary, let's use a robust method
                        
                        headers = {"Content-Type": "application/json"}
                        payload = {
                            "model": "llama-3.2-11b-vision-preview",  # Try latest available
                            "messages": [
                                {
                                    "role": "user",
                                    "content": [
                        ...
                        result = None
                        endpoints = [
                            "https://api.pollinations.ai/openai/chat/completions",
                            "https://image.pollinations.ai/openai/chat/completions"
                        ]
                        ...
                        if result:
                            ...
                            # Fallback: Ask user to describe or use text-only AI
                            st.warning("⚠️ Direct image analysis failed. Using text-based fallback...")
                            st.info("Please describe what you see in the image below, and I'll analyze it!")
                            
                            manual_desc = st.text_area(
                                "Describe the image:",
                                placeholder="e.g., A screenshot of code showing an error...",
                                key="mv_fallback"
                            )
                            
                            if st.button("🔍 Analyze Description", key="mv_fallback_btn"):
                                if manual_desc:
                                    client = get_ai_client()
                                    if client:
                                        full_prompt = f"User described an image: '{manual_desc}'. Task: {prompt_text}"
                                        response = client.chat.completions.create(
                                            model=Config.SMART_MODEL,
